...
@router.post("/suggest")
def suggest_transformation(source_id: int, target_id: int, source_table: str, target_table: str, db: Session = Depends(get_db)):
    """
    AI Suggests column mappings and structure casting logic between different source and target tables.
    """
    ...
    try:
        ...
        if source_table not in source_schema:
            raise HTTPException(status_code=400, detail=f"Table '{source_table}' not found in Source '{source_conn.name}'")
        if target_table not in target_schema:
            raise HTTPException(status_code=400, detail=f"Table '{target_table}' not found in Target '{target_conn.name}'")

        # Run AI matching on the columns
